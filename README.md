<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:4F46E5,50:7C3AED,100:4C1D95&height=250&section=header&text=BHOOMI%20MALVIYA&fontSize=48&fontColor=E9D5FF&animation=fadeIn&fontAlignY=38&desc=Full%20Stack%20Software%20Engineer%20%7C%20AI-Integrated%20Product%20Builder&descAlignY=55&descSize=18&descColor=C4B5FD" width="100%"/>

<a href="https://www.linkedin.com/in/Bhumimalviya05">
<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=20&duration=3000&pause=900&color=A78BFA&center=true&vCenter=true&width=650&lines=Shipping+Production-Grade+MERN+Applications;Engineering+AI-Integrated+Full-Stack+Systems;National+Finalist+%40+Smart+India+Hackathon+2024;Open+Source+Contributor+%40+Apache+Superset" alt="Typing SVG"/>
</a>

<br/><br/>

<img src="https://img.shields.io/badge/CGPA-9.17%2F10-6D28D9?style=for-the-badge&labelColor=1E1B2E"/>
<img src="https://img.shields.io/badge/B.Tech-Computer%20Science-6D28D9?style=for-the-badge&labelColor=1E1B2E"/>
<img src="https://img.shields.io/badge/Location-Bhopal%2C%20India-6D28D9?style=for-the-badge&labelColor=1E1B2E"/>

<br/><br/>

<a href="https://kiran-sales.vercel.app/"><img src="https://img.shields.io/badge/Live%20Project-8B5CF6?style=for-the-badge&logo=vercel&logoColor=white&labelColor=1E1B2E"/></a>
<a href="https://www.linkedin.com/in/Bhumimalviya05"><img src="https://img.shields.io/badge/LinkedIn-8B5CF6?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=1E1B2E"/></a>
<a href="mailto:mbhumi916@gmail.com"><img src="https://img.shields.io/badge/Email-8B5CF6?style=for-the-badge&logo=gmail&logoColor=white&labelColor=1E1B2E"/></a>
<a href="https://github.com/Bhumi0189"><img src="https://img.shields.io/badge/GitHub-8B5CF6?style=for-the-badge&logo=github&logoColor=white&labelColor=1E1B2E"/></a>

</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:4F46E5,100:4C1D95&height=2&width=100%"/>

## <img src="https://img.shields.io/badge/-01-1E1B2E?style=flat-square"/> About

I'm a Full Stack Software Engineer specializing in the **MERN stack**, focused on building production-grade, AI-integrated web systems rather than isolated demos. My work spans end-to-end ownership — architecture, API design, authentication, deployment, and performance optimization — most recently shipping a live e-commerce platform serving real customers with a **99/100 Lighthouse Performance score**.

I bring a **product engineering mindset**: every project I build is scoped around real users (students, NGOs, agents, shoppers) rather than just technical completeness. I integrate applied AI — recommendation logic, conversational assistants, classification, and gamified behavioral systems — into full-stack products, and contribute to open-source infrastructure at scale (Apache Superset).

```yaml
open_to:
  - Full Stack Developer roles (MERN)
  - Frontend Engineering roles (React / Next.js)
  - AI-Integrated Product Engineering
  - Open Source Collaboration
  - Internship / New-Grad Software Engineering positions
```

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:4F46E5,100:4C1D95&height=2&width=100%"/>

## <img src="https://img.shields.io/badge/-02-1E1B2E?style=flat-square"/> Tech Stack

**Languages**
<p><img src="https://skillicons.dev/icons?i=js,ts,java,python&theme=dark"/></p>

**Frontend**
<p><img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,html,css&theme=dark"/></p>

**Backend & Databases**
<p><img src="https://skillicons.dev/icons?i=nodejs,express,mongodb&theme=dark"/></p>

**Cloud, DevOps & Tooling**
<p><img src="https://skillicons.dev/icons?i=vercel,git,github,postman,vscode&theme=dark"/></p>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:4F46E5,100:4C1D95&height=2&width=100%"/>

## <img src="https://img.shields.io/badge/-03-1E1B2E?style=flat-square"/> Applied AI Expertise

<div align="center">

| Domain | Proficiency | Details |
|---|---|---|
| AI-Powered Search & Recommendations | Applied / Integration | Preference-matching search engine for real estate listings (SATYA Constructions) |
| Conversational & Voice AI | Applied / Integration | Voice-interactive study companion with personalized plans (EduPath AI) |
| Computer Vision (Classification) | Applied / Integration | AI-based waste classification module (EcoVerse) |
| Behavioral & Gamification Logic | Applied | Engagement systems — streaks, badges, leaderboards driving retention (EcoVerse, EduPath AI) |

</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:4F46E5,100:4C1D95&height=2&width=100%"/>

## <img src="https://img.shields.io/badge/-04-1E1B2E?style=flat-square"/> Featured Projects

<details open>
<summary><b>🛒 Kiran Sales — Production E-Commerce Platform</b></summary>
<br/>

Live, production MERN e-commerce platform built and maintained solo for a real medical apparel business — from requirements through deployment.

| | |
|---|---|
| **Stack** | React.js, Node.js, Express.js, MongoDB, Tailwind CSS, Vercel |
| **Scale** | Live production system serving real customers daily |
| **Performance** | 99/100 Lighthouse Performance · 100/100 Best Practices · 91/100 SEO |
| **Security** | JWT-based authentication, secured checkout & order workflows |
| **Impact** | Reduced manual order handling via bulk order management tooling |
| **Repository** | [github.com/Bhumi0189/kiran-sales](https://github.com/Bhumi0189/kiran-sales) · [Live](https://kiran-sales.vercel.app/) |

Sole-developer ownership of the full product lifecycle: responsive catalog, cart workflows, authentication, and a reusable component system, deployed and optimized on Vercel.

</details>

<details>
<summary><b>🌍 EcoVerse — AI-Powered Environmental Education Platform</b></summary>
<br/>

AI-integrated sustainability platform turning environmental education into an interactive, gamified experience for students, NGOs, and universities.

| | |
|---|---|
| **Stack** | MERN, AI Integration, Gamification, AR |
| **Scale** | Multi-role platform — student, NGO, university, admin dashboards |
| **Performance** | Real-time analytics across all dashboard roles |
| **Security** | Role-based access across four distinct user types |
| **Impact** | Gamified engagement engine (eco-points, rewards, leaderboards) aligned with SDGs & NEP 2020 |
| **Repository** | [github.com/Bhumi0189/EcoVerse](https://github.com/Bhumi0189/EcoVerse) · [Live](https://eco-verse-lovat.vercel.app/) |

Integrates AI-driven eco-coaching, waste classification, and AR-based eco hunts into a single collaborative platform.

</details>

<details>
<summary><b>📚 EduPath AI — Intelligent Study Companion</b></summary>
<br/>

AI-powered study platform delivering personalized study plans through voice interaction and gamified learning mechanics.

| | |
|---|---|
| **Stack** | MERN, AI Integration, Voice Assistant |
| **Scale** | Full learning companion — study plans, career guidance, engagement systems |
| **Performance** | Real-time personalization based on student scores & interests |
| **Security** | Standard authenticated user sessions |
| **Impact** | Career-guidance engine mapping scores/interests to actionable paths |
| **Repository** | [github.com/Bhumi0189/EduPathAI](https://github.com/Bhumi0189/EduPathAI) |

Daily streak and badge system engineered to drive consistent, repeat engagement.

</details>

<details>
<summary><b>🏗️ SATYA Constructions — Real Estate Platform</b></summary>
<br/>

Responsive real estate platform with smart property search, virtual tours, and AI-driven recommendations.

| | |
|---|---|
| **Stack** | MERN, AI Search, Location Intelligence |
| **Scale** | Full listings platform with agent-facing lead tooling |
| **Performance** | Automated preference-matching search, no manual filtering |
| **Security** | Standard authenticated workflows |
| **Impact** | Automated lead-generation workflows delivering formatted inquiries instantly to agents |

</details>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:4F46E5,100:4C1D95&height=2&width=100%"/>

## <img src="https://img.shields.io/badge/-05-1E1B2E?style=flat-square"/> Experience

**Freelance Full Stack Developer** · Kiran Sales (Medical Apparel E-Commerce)
`Oct 2025 — Present`

Engaged as sole developer to design, build, and deploy a production-ready MERN e-commerce platform for a real business, now live and serving customers.

- Architected and shipped a responsive product catalog, secure authentication, cart workflows, and bulk order management
- Built a reusable React + Tailwind component system for consistent cross-device UI
- Deployed and optimized the application on Vercel, achieving a 99/100 Lighthouse Performance score

`React.js` `Node.js` `Express.js` `MongoDB` `Tailwind CSS` `Vercel` `JWT`

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:4F46E5,100:4C1D95&height=2&width=100%"/>

## <img src="https://img.shields.io/badge/-06-1E1B2E?style=flat-square"/> Achievements

<div align="center">

| Recognition | Details |
|---|---|
| 🏆 Smart India Hackathon 2024 — National Finalist | Selected among top teams nationwide for SIH 1620, an Intelligent Queuing Model for OPD healthcare optimization |
| 🔧 Open Source Contributor — Apache Superset | Merged PR #37262 covering TypeScript migration and bug fixes, reviewed by the global Superset developer community |
| 📜 Full Stack Development & Java with DSA | Apna College — hands-on MERN stack, DSA, and scalable web application training |

</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:4F46E5,100:4C1D95&height=2&width=100%"/>

## <img src="https://img.shields.io/badge/-07-1E1B2E?style=flat-square"/> GitHub Contribution Graph

<div align="center">

<img src="https://raw.githubusercontent.com/Bhumi0189/Bhumi0189/output/github-contribution-grid-snake-dark.svg" width="100%"/>

</div>


<img src="https://capsule-render.vercel.app/api?type=rect&color=0:4F46E5,100:4C1D95&height=2&width=100%"/>

## <img src="https://img.shields.io/badge/-08-1E1B2E?style=flat-square"/> Contribution Activity

<div align="center">
<img src="https://github-readme-activity-graph.vercel.app/graph?username=Bhumi0189&theme=react-dark&bg_color=0D1117&color=A78BFA&line=8B5CF6&point=C4B5FD&hide_border=true"/>
</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:4F46E5,100:4C1D95&height=2&width=100%"/>

## <img src="https://img.shields.io/badge/-09-1E1B2E?style=flat-square"/> GitHub Analytics

<div align="center">
<img src="https://github-stats-extended.vercel.app/api?username=Bhumi0189&show_icons=true&count_private=true&hide_border=true&bg_color=0D1117&title_color=A78BFA&icon_color=8B5CF6&text_color=C4B5FD" height="165"/>
<img src="https://github-stats-extended.vercel.app/api/top-langs/?username=Bhumi0189&layout=compact&hide_border=true&bg_color=0D1117&title_color=A78BFA&text_color=C4B5FD" height="165"/>
</div>

<div align="center">
<img src="https://github-readme-streak-stats.herokuapp.com/?user=Bhumi0189&hide_border=true&background=0D1117&ring=8B5CF6&fire=A78BFA&currStreakLabel=C4B5FD"/>
</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:4F46E5,100:4C1D95&height=2&width=100%"/>

## <img src="https://img.shields.io/badge/-10-1E1B2E?style=flat-square"/> GitHub Achievements

<p align="left">
  <a href="https://github.com/Bhumi0189?tab=achievements">
    <img src="https://github.githubassets.com/images/modules/profile/achievements/yolo-default.png" width="85" alt="YOLO"/>
  </a>
  &nbsp;&nbsp;
  <a href="https://github.com/Bhumi0189?tab=achievements">
    <img src="https://github.githubassets.com/images/modules/profile/achievements/pull-shark-default.png" width="85" alt="Pull Shark"/>
  </a>
</p>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:4F46E5,100:4C1D95&height=2&width=100%"/>

## <img src="https://img.shields.io/badge/-11-1E1B2E?style=flat-square"/> Current Focus

```yaml
learning:
  - Advanced system design for scalable MERN architectures
  - Deeper AI/ML integration patterns for production apps
building:
  - Expanding EcoVerse and EduPath AI feature sets
  - Refining freelance client infrastructure and performance
exploring:
  - TypeScript-first backend architecture
  - Open source contributions beyond Apache Superset
open_to:
  - Full Stack Developer & Frontend Engineer roles
  - AI-integrated product engineering opportunities
  - Internship / new-grad Software Engineering positions
```

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:4F46E5,100:4C1D95&height=2&width=100%"/>

## <img src="https://img.shields.io/badge/-12-1E1B2E?style=flat-square"/> Connect

<p align="center">
<a href="mailto:mbhumi916@gmail.com"><img src="https://img.shields.io/badge/Gmail-8B5CF6?style=for-the-badge&logo=gmail&logoColor=white&labelColor=1E1B2E"/></a>
<a href="https://www.linkedin.com/in/Bhumimalviya05"><img src="https://img.shields.io/badge/LinkedIn-8B5CF6?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=1E1B2E"/></a>
<a href="https://github.com/Bhumi0189"><img src="https://img.shields.io/badge/GitHub-8B5CF6?style=for-the-badge&logo=github&logoColor=white&labelColor=1E1B2E"/></a>
<a href="https://kiran-sales.vercel.app/"><img src="https://img.shields.io/badge/Portfolio-8B5CF6?style=for-the-badge&logo=vercel&logoColor=white&labelColor=1E1B2E"/></a>
</p>

<div align="center">

<i>"Engineering products people actually use — not just code that compiles."</i>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:4C1D95,50:7C3AED,100:4F46E5&height=150&section=footer" width="100%"/>

</div>
